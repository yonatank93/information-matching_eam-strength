...
# Convert the scale list 0 to "inf"
try:
    idx_inf = scale_list.index(0)
    scale_list.remove(0)
    scale_list.append("inf")
except ValueError:
    pass
# Random initial starting points
nsamples = utils.nsamples + 1  # Including the unperturbed parameters
    ...
    for folder in dirpath.iterdir():
        ...
        if sample_dir.exists():
            ...
            if check_file.exists():
                ...
                # Parse
                opt_x = opt["x"]
                opt_cost = opt["cost"]  # lstsq cost with regularization
                # Save
...
cost_data = []
cost_reg = []
for key, val in opt_training_results.items():
    residuals = val["info"]["fun"]
    if key == "inf":  # No regularization case
        npreds = len(residuals)
    else:
        npreds = len(residuals) - nparams
...
if args.show:
    ...
    if "inf" in lambdas:
        ...
        xtick_labels = [f"{l:0.1e}" for l in lambdas]
    plt.figure()
