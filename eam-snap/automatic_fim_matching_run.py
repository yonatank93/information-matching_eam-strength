...
# -----------------------
# None - All flags are covered by general settings
...
# ---------------------
# None - All flags are covered by general settings, w0 flag is handled by
#        NITERS and NBURNINS settings.

# Submit training jobs settings
# -----------------------------
submit_training_settings = settings["submit_training"]
# Ground truth reference data
GT_FILE = submit_training_settings["gt_file"]
...
for ii in range(NITERS):
    ...
    check_file = TARGET_DIR / "FIM_candidates" / "score_results.xyz"
    ...
    if not check_file.exists():
        ...
        sync_calc(TARGET_DIR / "FIM_target" / "fim_target.npy")
    ...
    if not check_file.exists():
        ...
        # Run the FIM-matching calculation
        output = run_python_command(python_command, True)
    ...
    if not check_file.exists():
        ...
        # time.sleep(60)  # Just wait while the jobs are completing
